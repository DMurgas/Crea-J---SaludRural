...
case 1: // alta
    ...
    $consulta = "SELECT id, nombre, descripcion, lugar,pass FROM hospitales ORDER BY id DESC LIMIT 1";
    $resultado = $conexion->prepare($consulta);
    $resultado->execute();
    ...
    case 2: //modificación
        ...
        $consulta = "SELECT id, nombre, descripcion, lugar,pass FROM hospitales WHERE id='$id' ";       
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();
        $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
        break;        
